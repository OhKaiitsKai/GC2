...
cbuffer cbChangerEveryFrame : register(b5)
{
...
struct PS_Input
{
    ...
    float waterTranslation : TEXCOORD6;
...
PS_Input VS_Main(VS_Input vertex)
{
    ...
    vsOut.waterTranslation = waterTranslation;
...
float4 PS_Main(PS_Input pix) : SV_TARGET
{
    ...
    pix.tex0.x += pix.waterTranslation;

    textureColor = float4(0.3, 0.5, 0.7, 1);
	color = float4(0.5, 0.5, 0.5, 1);// ambient color

    bumpMap = normalMap.Sample(colorSampler, pix.tex0);
    //bumpMap = (bumpMap * 2.0) - 1;
