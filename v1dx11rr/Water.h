...
public:
	...
	Water(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* specularTexturePath, WCHAR* normalTexturePath)
	{
		...
		CargaParametros(ModelPath, specularTexturePath, normalTexturePath);
	}
	...
	bool CargaParametros(char* ModelPath, WCHAR* specularTex, WCHAR* normalTex)
	{
		HRESULT d3dResult;
		...
		vertexDesc.ByteWidth = sizeof(VertexObj) * m_ObjParser.m_nVertexCount; //Cantidad de geometría a pasar

		D3D11_SUBRESOURCE_DATA resourceData;
		ZeroMemory(&resourceData, sizeof(resourceData));
		resourceData.pSysMem = m_ObjParser.m_pVertex;//Le pasamos los vertices del modelos

		//Aquí creamos el buffer que almacenará el modelo  con su descripción, los recursos y la variable donde lo guardaremos
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &waterTrasCB);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float xpos, float ypos, float zpos, D3DXVECTOR3 posCam,
		float specForce, float rot, char angle, float scale, bool movCam, int tipoCam, float waterTras)
	{
		...
		d3dContext->PSSetShaderResources(0, 1, &specMap);
		...
		if (tipoCam == 1) {
			D3DXMatrixTranslation(&traslacionDeCamara, -2.8, 0.0, -0.8);
		}
		else if (tipoCam == 3) {
			D3DXMatrixTranslation(&traslacionDeCamara, -3.5, 0.0, -15.0);
		}
		...
		D3DXMATRIX scaleMat;
		D3DXMatrixScaling(&scaleMat, scale, scale * 1.7, scale);
		...
		d3dContext->UpdateSubresource(waterTrasCB, 0, 0, &waterTras, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(4, 1, &waterTrasCB);
