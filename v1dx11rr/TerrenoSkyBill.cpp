                   ...
                   int nCmdShow)
{
    ...
    hWnd = CreateWindowEx(WS_EX_APPWINDOW,
                          L"DXRR_E1",
                          L"PLANTILLA PROYECTO", //to_string(dxrr->camara->posCam.x).c_str()
                          WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP | WS_SYSMENU | WS_CAPTION | CDS_FULLSCREEN,
        posX,
        posY,
        SCREEN_X,
        SCREEN_Y,
    ...
    while(TRUE)
    {
        ...
        //wchar_t* coords = new wchar_t(dxrr->camara->posCam.x);
        //wchar_t* coordsz = new wchar_t(dxrr->camara->posCam.z);
        //wcsncat(coords, L", ", 5);
        //wcsncat(coords, coordsz, 9);

        //SetWindowText(hWnd, coords);
    }
    ...
    switch(message)
    {
        ...
        case WM_MOUSEMOVE: {
           ...
           if (keyboardData[DIK_M] & 0x80) {
               //mostrar imagen
               dxrr->status2 = true;
               //dxrr->nota_1_imagen->Draw(0, 0, 8, 12, 12);
              // dxrr->uno = true;
           }
        ...
        case WM_CHAR: {
            ...
            case 'P':
                //go to the car position
                //camaraxd->setCondicion(true);
                //MessageBox(0, L"Primera persona", L"Cambia a primera persona", MB_OK);
                break;
