...
float4 PS_Main(PS_Input pix) : SV_TARGET
{
	...
	// Creamos Las Nuevas Coordenadas Para la Textrura del Agua, Timer y Direccion 
    float2 NuevasCoord = float2(pix.tex0.x + Timer, pix.tex0.y);

    textureColor = colorMap.Sample(colorSampler, NuevasCoord); ////Remplazamos
    color = float4(0.2, 0.2, 0.2, 1); // ambient color

    specular = float4(0.0, 0.0, 0.0, 1.0); //specular color
    specularMap = specMap.Sample(colorSampler, NuevasCoord); /////Remplazamos
    ...
    if (lightIntensity > 0)
    {
        ...
        reflection = normalize(6 * lightIntensity * pix.normal - lightDir);
