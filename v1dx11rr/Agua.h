...
public:
	Agua(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* specularTexturePath, float _posX, float _posZ)
	{
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex)
	{
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &TimerCB);
	...
	//pintar awa// 
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, float Timer)
	{
		...
		d3dContext->UpdateSubresource(TimerCB, 0, 0, &Timer, 0, 0);
		...
		//////
		// Pasamos el Timer al Pixel Shader 

		d3dContext->PSSetConstantBuffers(5, 1, &TimerCB);
